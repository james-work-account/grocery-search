...
def get_tesco_searches() -> str:
    print('Searching Tesco...')
    return search_online_shop(
        ...
        offer_selector='div > div.product-tile.has-promotion > div > div.promotions-wrapper.hidden-medium > ul > li > a > div > span.offer-text, .info-message',
        title_css_selector='a[data-auto="product-tile--title"]',
    )


def get_morrisons_searches() -> str:
...
def get_sainsburys_searches() -> str:
...
def search_online_shop(url: str, not_found_css_selector: str, items_list_selector: str,
    ...
    try:
        ...
        if len(driver.find_elements_by_css_selector(not_found_css_selector)) == 0:
            ...
            return t.get_string(sortby='Price', sort_key=lambda row: format_price(row[0]))
        else:
            return f'No results found for {SEARCH_TERM}'
    except (NoSuchElementException, TimeoutException):
        return f'Error finding product: {SEARCH_TERM}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--number-of-items', '-n', help='The maximum number of items to return (default 10)', type=int,
                        default=10)
    parser.add_argument('--browser-not-headless', '-b',
                        help='Specify if the browser should run in Headless mode or not (default True)',
    ...
    options = Options()
    if not args.browser_not_headless:
    ...
    # Search shops
    print_results([
        Result('TESCO', get_tesco_searches()),
        Result('MORRISONS', get_morrisons_searches()),
        Result('WAITROSE', get_waitrose_searches()),
        Result('ALDI', get_aldi_searches()),
        Result('SAINSBURYS', get_sainsburys_searches()),
        Result('ASDA', get_asda_searches()),
    ])
